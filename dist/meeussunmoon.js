...
(function (global, factory) {
  ...
  (global = global || self, factory(global.MeeusSunMoon = {}, global.luxon));
  ...
  var datetimeToJD = function datetimeToJD(datetime) {
    ...
    var D = datetime.day + (datetime.hour + (datetime.minute + datetime.second / 60) / 60) / 24;
    ...
    var gregorianCutoff = luxon.DateTime.fromISO('1582-10-15T12:00:00Z', {
      zone: 'UTC'
    });
    var B = 0;
  ...
  var JDToDatetime = function JDToDatetime(JD) {
    ...
    var hour = Math.floor((fracDay - day) * 24);
    var minute = Math.floor(((fracDay - day) * 24 - hour) * 60);
    var second = Math.floor((((fracDay - day) * 24 - hour) * 60 - minute) * 60);
    var month = E - 1;
    ...
    return luxon.DateTime.fromISO('2000-01-01T12:00:00Z', {
      zone: 'UTC'
  ...
  var approxK = function approxK(datetime) {
    var year = datetime.year + datetime.month / 12 + datetime.day / 365.25;
  ...
  /** See AA p144 */
  var sunMeanAnomaly = [357.52772, 35999.050340, -0.0001603, -1 / 300000];
  /** See AA p163 Eq 25.2 */
  ...
  var moonArgumentOfLatitude = [93.27191, 483202.017538, -0.0036825, 1 / 327270];
  /** See AA p144 */

  var moonAscendingNodeLongitude = [125.04452, -1934.136261, 0.0020708, 1 / 450000];
  /** See AA p144 */

  var moonMeanAnomaly = [134.96298, 477198.867398, 0.0086972, 1 / 56250];
  /** See AA p144 */
  ...
  var sunTransit = function sunTransit(datetime, L) {
    var timezone = datetime.zone;
    var transit = datetime.set({
      hour: 0,
      minute: 0,
      second: 0
    }).setZone('UTC', {
  ...
  var sunRiseSet = function sunRiseSet(datetime, phi, L, flag) {
    ...
    if (m > 0) {
      suntime = suntime.plus({
        seconds: Math.floor(m * 3600 * 24 + 0.5)
      });
    } else {
      suntime = suntime.minus({
        seconds: Math.floor(m * 3600 * 24 + 0.5)
      });
    }
  ...
  var handleNoEventCase = function handleNoEventCase(date, errorCode, hour) {
    var minute = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : 0;

    if (exports.returnTimeForNoEventCase) {
      var returnDate = date.set({
        hour: hour,
        minute: minute,
        second: 0
      }).plus({
        minutes: date.isInDST ? 60 : 0
      });
  ...
  var altitude = function altitude(phi, delta, H) {
    return rad2deg(Math.asin(sind(phi) * sind(delta) + cosd(phi) * cosd(delta) * cosd(H)));
  };
  /**
  ...
  var sunApparentRightAscension = function sunApparentRightAscension(T) {
    var Omega = moonAscendingNodeLongitude$1(T);
  ...
  var apparentSiderealTimeGreenwich = function apparentSiderealTimeGreenwich(T) {
    var theta0 = meanSiderealTimeGreenwich(T);
  ...
  var nutationInLongitude = function nutationInLongitude(T) {
    ...
    var M = sunMeanAnomaly$1(T);
    var MPrime = moonMeanAnomaly$1(T);
    var F = moonArgumentOfLatitude$1(T);
    var Omega = moonAscendingNodeLongitude$1(T);
    var DeltaPsi = 0;
    var sineArg;
  ...
  var moonArgumentOfLatitude$1 = function moonArgumentOfLatitude$1(T) {
    var F = polynomial(T, moonArgumentOfLatitude);
  ...
  var moonAscendingNodeLongitude$1 = function moonAscendingNodeLongitude$1(T) {
    var Omega = polynomial(T, moonAscendingNodeLongitude);
  ...
  var moonMeanAnomaly$1 = function moonMeanAnomaly$1(T) {
    var MPrime = polynomial(T, moonMeanAnomaly);
  ...
  var sunMeanAnomaly$1 = function sunMeanAnomaly$1(T) {
    var M = polynomial(T, sunMeanAnomaly);
  ...
  var truePhase = function truePhase(k, phase) {
    ...
    var M = sunMeanAnomaly$2(T, k);
    var MPrime = moonMeanAnomaly$2(T, k);
    var F = moonArgumentOfLatitude$2(T, k);
    var Omega = moonAscendingNodeLongitude$2(T, k);
  ...
  var commonCorrections = function commonCorrections(A) {
    return 0.000325 * sind(A[1]) + 0.000165 * sind(A[2]) + 0.000164 * sind(A[3]) + 0.000126 * sind(A[4]) + 0.000110 * sind(A[5]) + 0.000062 * sind(A[6]) + 0.000060 * sind(A[7]) + 0.000056 * sind(A[8]) + 0.000047 * sind(A[9]) + 0.000042 * sind(A[10]) + 0.000040 * sind(A[11]) + 0.000037 * sind(A[12]) + 0.000035 * sind(A[13]) + 0.000023 * sind(A[14]);
  };
  /**
   * Calculates the corrections to the planetary arguments for the moon phases
  ...
  var options = function options(_options) {
    ...
    if (typeof _options.returnTimeForNoEventCase === 'boolean') {
      exports.returnTimeForNoEventCase = _options.returnTimeForNoEventCase;
    }
  ...
  var formatCI = function formatCI(datetime, formatString) {
    var noEventCode = datetime.errorCode;
    var datestring = datetime.toFormat(formatString);
  ...
  var yearMoonPhases = function yearMoonPhases(year, phase) {
    var timezone = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'UTC';
    var yearBegin = luxon.DateTime.fromObject( // eslint-disable-next-line sort-keys
    {
      year: year,
      month: 1,
      day: 1,
    ...
    var yearEnd = luxon.DateTime.fromObject( // eslint-disable-next-line sort-keys
    {
      year: year + 1,
      month: 1,
      day: 1,
    ...
    for (var i = 0; i < 15; i++) {
      ...
      moonDatetime = JDToDatetime(JDE).setZone(timezone); // now use that to calculate deltaT

      deltaT = DeltaT(moonDatetime);

      if (deltaT > 0) {
        moonDatetime = moonDatetime.minus({
          seconds: Math.abs(deltaT)
        });
      } else {
        moonDatetime = moonDatetime.plus({
          seconds: Math.abs(deltaT)
        });
      }
